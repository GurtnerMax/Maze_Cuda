 ...
 * Interet:
 *
 * 	On utilise un lock lorsque l'operation qui doit etre synchroniser ne possede pas d'operateur atomic (comme atomicADD, ...),
 * 	ou lorsqui'il s'agit de plusieurs operations a synchroniser (joue alors le role de section critique)
 *
 * Note :
 *
 * 	Lock ne laisse aucune trace coter host, il s'instancie only coter device: Code moins invasif
 ...
 * 	__device__ volatile int mutex=0;	// Attention a l'initialisation
 *
 ...
 *                       	8  threads passent  � gauche dans b
 *
 *                       Model 1 (Faux):
 *
...
class Lock
	 ...
	 */
	__device__ Lock(volatile int* ptrDevMutexGM)
	    {
	...
	void lock(void)
	    ...
	    // 		Action :	compare ptr avec a, si egale affecte b a ptr, renvoie ptr

	    // Tant que ptrDevMutexGM!=0 le thread cuda boucle sur while
	    // Des qu'il vaut 0, il met le mutex ptrDevMutexGM a 1  (CAS=Compare And Swap) CAS(variable,valeurComparaison,valeurSwap)
	...
	void unlock(void)
	    ...
	    //
	    //		*ptrDevMutexGM=0;
	    //		Aucun thread en competition ici. L'affectation n'a pas besoin d'etre atomique.
	    //		Solution satisfaisante.
	    //
	    // Solution 2 (prefered for symetric approach et robustess)(Perte de performance?)
	    //
	    //		 atomicExch((int*)ptrDevMutexGM, 0);

	    //v1
	    //*ptrDevMutexGM = 0;	    // tester ok

	    //v2
	    atomicExch((int*)ptrDevMutexGM, 0);// tester ok (Echange et renvoie la valeur originale)

	    // new (aded from cuda 10 et from rtx 2080ti)
	    __threadfence();	    // ko pour v1 et v2 si pas la!
	    // __threadfence :	function stalls current thread until its writes to global memory are guaranteed
	    // 			to be visible by all other threads in the grid.
	    }
